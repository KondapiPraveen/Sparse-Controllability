...
FS = 0.1:0.1:1;
FS = union(FS,1);
NSys = 1; % # Independent Trials

N = 60:50:100; % State, Input Dimensions % Setup - 1 
e_0 = 1e-5; e_01 = 1e-30;
%{
% Setup - 2
N = 20:20:80; m = 100;
...
Opt = zeros(ln,lg,NSys);
tic;

for j=1:ln
    n = N(j);
    m = n; % Setup - 1;
    %MA = 2*rand(n,n,NSys)-1;
    MA = Erdos_Renyi_2(n,NSys);
    MB = rand(n,m,NSys);
    S_ki = zeros(n,m);
    %B = eye(n); % m=n
    ...
    for l=1:NSys
        ...
        for k=1:lg
            ...
            if s==m
               ...
               %[~,~,S_ki] = FullBLI(A,B,n,s,e_0);
               %[S_g,Giopt(j,k,l)] = GreedyScheduling_Aopt_FullB(R,IW_S,m,S_ki,n,s);
               %Opt(j,k,l) = trace(R(:,S_g)*R(:,S_g).');
            ...
            %}
            [~,~,Giopt(j,k,l)]=SparseScheduling_2(R,S_ki,m,n,s,e_01);
        end
    end
end
toc;
...
for l=1:NSys
    ...
    nopt(:,:,1) = Opt(:,:,l)./Opt(:,end,l);
    %nGSopt(:,:,l) = GSiopt(:,:,l)./GSiopt(:,end,l);
end
Topt = sum(nopt,3)/NSys;
...
colororder(Colors)
loglog(S/m,Topt.','LineWidth',3,'Marker','s','MarkerSize',10)
grid on; hold on
%plot(S/m,(m./S).*(1+exp(-S/m)),'-.k','LineWidth',3,'Marker','o','MarkerSize',10); str = "$\frac{m}{s}(1+e^{-\frac{s}{m}})$";
...
loglog(S/m,S/m,'-.k','LineWidth',3,'Marker','o','MarkerSize',10); str = "$\frac{m}{s}$";
%h1 = legend([string(strcat('n = ',num2str(N.')));str],'NumColumns',2);
