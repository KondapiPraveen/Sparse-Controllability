...
% Initialization
FS = [0.1,0.2,0.3,0.4,0.7,0.9,1];
Pr = [0,0.3,0.6];
NSys = 100; % # Independent Trials

n = 100; % State, Input Dimensions % Setup - 1 
B = eye(n); m=n; S = ceil(FS*m); 
e_0 = 1e-5;
%{
% Setup - 2
N = 20:20:80; m = 100;
...
lg = length(FS);
lp = length(Pr);
Giopt = zeros(lp,lg,NSys); % Time Varying Support
%LBnd = zeros(lp,lg,NSys); % 1 - Lower Bound - Validity of Representation
%UBnd = zeros(lp,lg,NSys); % 2 - Upper Bound - Validity of Representation
GSiopt = zeros(lp,lg,NSys); % Fixed Support

tic;
for j=1:lp
    p = Pr(j); 
    %m = n; % Setup - 1;
    MA = Erdos_Renyi_3(n,p,NSys);
    %MB = rand(n,m,NSys);
...
%%
nGopt = zeros(lp,lg,NSys);
nGSopt = zeros(lp,lg,NSys);
for l=1:NSys
...
grid on
legend(strcat('p = ',num2str(Pr.')),'NumColumns',2)
...
h1 = legend([string(strcat('p = ',num2str(Pr.')));str],'NumColumns',2);
...
Colors = repmat(DefaultColors(1:lp,:),3,1);
colororder(Colors)
