...
for j=1:ln
    ...
    parfor l=1:NSys
        A = MA(:,:,l); B = MB(:,:,l);
        R = CtrlMatrix(A,B,n); W = R*R.';
        IW = W\eye(n); AvgEng = trace(IW);
        for k=1:lg
            s=S(k);
            if s==m
                Giopt(j,k,l) = trace(IW);
               ...
               E = (s/m)*W - R(:,S_g)*R(:,S_g).';
               UBnd(j,k,l) = m/s + (sqrt(n)*((m/s)^2)*norm(IW*E*((eye(n)-(m/s)*IW*E)\IW),'fro'))/AvgEng;
               inv(A)*B;
...
loglog(S/m,m./S,'-.k','LineWidth',3,'Marker','o','MarkerSize',10);
str = "$\frac{m}{s}$"; 
h1 = legend([string(strcat('n = ',num2str(N.')));string(strcat('UBnd n = ',num2str(N.')));str],'NumColumns',2);
set(h1,'Interpreter','latex','FontSize',26)
set(gca,'FontSize',20,'FontWeight','bold')
