...
s_greedy_cost = s_greedy_cost/Aopt(1,lg); 
s_greedy_mcmc_cost = s_greedy_mcmc_cost/Aopt(1,lg); Aopt = Aopt/Aopt(1,lg); 
%% Plotting
figure();
x_axis = S/m;
loglog(x_axis,Aopt(5,:),'r-','LineWidth',2,'Marker','d','MarkerSize',10,'DisplayName','Random (Weighted)')
grid on; hold on;
loglog(x_axis,Aopt(4,:),'r--','LineWidth',2,'Marker','d','MarkerSize',10,'DisplayName','Random (Unweighted)')
loglog(x_axis,Aopt(3,:),'--','LineWidth',2,'Marker','+','MarkerSize',10,'DisplayName','Deterministic (Unweighted)','Color',"#009900")
