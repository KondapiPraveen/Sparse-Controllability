...
NSys = 1; n = 40; m = 40; p = 40; sig_v = 1e-2; sig_w = sig_v;
S = floor((0.1:0.1:0.5)*m); NoisedB = 20*log10(sig_v);
%S = m;
ls = length(S);
...
for i=1:NSys
    ...
    for l=1:ls
        ...
        X1(:,1) = x0+R_x*(initPertb); X2(:,1) = X1(:,1); % Reachability
        ...
        for j=1:K
            ...
            %end
                ...
                [X_est2(:,j), P2] = KF(A,B,C,V,W,P2,X_est2(:,j-1),y2,u_pomp2);
            end
...
ylabel("OMP $10\log{\bf E ||xf-x||_2^2}$",'Interpreter','latex');
xlabel('Time Steps K');
