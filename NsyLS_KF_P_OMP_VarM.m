...
M = 80:16:240; lm = length(M);
NSys = 100; n = 80; p = n; sig_v = 1e-2; sig_w = sig_v;
...
UOMP = zeros(ls,lm,K,NSys); UPOMP = zeros(ls,lm,K,NSys);
    ...
    parfor i=1:NSys 
        ...
        for l=1:ls
            ...
            for j=1:K
                ...
                u_omp = OMP(B,x_hat1,s);
                %u_omp = cosamp(B,x_hat1,s,tol,max_itr);
...
NMSE1 = 10*log10(sum(NMSEi1(:,:,K+1,:),4)/NSys); NMSE2 = 10*log10(sum(NMSEi2(:,:,K+1,:),4)/NSys);
...
xlim([80, 240]); %xticks([0,5,10,15,20,25])
%% POMP for S
%{
figure();
